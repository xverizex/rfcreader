...
int update ( )
{
	...
	char *p_buffer_file = strstr (buffer, "\r\n\r\n");
	p_buffer_file += 4;

	int off_size = p_buffer_file - buffer;
	off_size = size_buffer - off_size;

	fprintf ( stderr, "downloading...\n" );
	/* открыть файл для записи */
	const char name[] = "/tmp/RFC-all.tar.gz";
	FILE *out = fopen ( name, "w" );
	int rret = fwrite (p_buffer_file, 1, off_size, out);
	if (rret == -1) {
		perror ("fwrite:");
		return 0;
	}
	...
	/* блок для сохранения файла */
	{
		...
		char *file = calloc (max_recv_size + 1, 1); //[ max_recv_size + 1 ];
		int get = off_size;
		...
		int cur_time = 0;
		/* записать в файл */
		printf ( "total downloaded: %4ld%%\r", result );
		do 
		{
			get = SSL_read ( ssl, file, max_recv_size );
			if ( get <= 0 ) break;
			percent += get;
			result = percent * 100.0 / total_size;
			printf ( "total downloaded: %4ld%%", result );
			fflush ( stdout );
			printf ("\r");
			int rr = fwrite ( file, 1, get, out );
			if (rr == -1) {
				perror ("fwrite out:");
			}
		}
